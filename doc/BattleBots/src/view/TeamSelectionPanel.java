...
//    defaultList.add("variable moved ; ( have i moved? )");
//    defaultList.add(": moved? moved ? ; ");
//    defaultList.add("moved false ! ;");
//    defaultList.add(": firstMove ( move to center first ) moved ? if ( already moved ) else move move move true moved ! then ;");
//    defaultList.add("variable shot ;  ( have I shot this play? )");
//    defaultList.add(": canShoot? ( --b ) ( shot available? ) shot ? ; ");
//    defaultList.add(": shoot!! ( id ir -- ) ( shoot if allowed ) canShoot? if drop drop ( remove ir id ) else shoot! ( really shoot ) shot true ! ( remember it ) then then ; ");
//    defaultList.add(": doNotShoot ( id ir -- ) ( pretend shoot ) drop drop ; ");
//    defaultList.add(": enemy? ( s--b ) ( decide if enemy ) team <> ;");
//    defaultList.add(": nonZeroRange? ( i --bi ) dup 0 <> ; ");
//    defaultList.add(": tryShooting! ( ih id ir st -- ) enemy? swap nonZeroRange? rot and if shoot!! else doNotShoot then drop ( remove ih ) ;");
//    defaultList.add(": shootEveryone ( try shot at all targets ) scan! 1 - dup 0 < if ( no one to shoot at ) else 0 do identify! tryShooting! loop then ;");
//    defaultList.add(": play ( -- ) false moved ! firstMove shot false ! ( prepare to shoot ) shootEveryone ; ");
//    defaultRobot.put("code", defaultList);
    defaultList.add("variable moved ; ( have i moved? )");
    defaultList.add(": moved? moved ? ; ");
    defaultList.add("moved false ! ");
    defaultList.add(": firstMove ( align along left edge ) moved? if ( already moved ) else 5 turn! then ;");
    defaultList.add(": edgeMove ( -- ) ( move along the edge ) 0 check! .\" OUT OF BOUNDS \" = if 1 turn! else ( ) then move ; ");
    defaultList.add(": noMovesLeft? ( --b ) ( no moves left? ) movesLeft 0 <> ; ");
    defaultList.add("variable shot ;  ( have I shot this play? )");
    defaultList.add(": canShoot? ( --b ) ( shot available? ) shot ? ; ");
    defaultList.add(": shoot!! ( id ir -- ) ( shoot if allowed ) canShoot? if drop drop ( remove ir id ) else shoot! ( really shoot ) shot true ! ( remember it ) then then ; ");
    defaultList.add(": doNotShoot ( id ir -- ) ( pretend shoot ) drop drop ; ");
    ...
    defaultList.add(": tryShooting! ( ih id ir st -- ) enemy? swap nonZeroRange? rot and if shoot!! else doNotShoot then drop ( remove ih ) ;");
    defaultList.add(": shootEveryone ( try shot at all targets ) scan! 1 - dup 0 < if ( no one to shoot at ) else 0 do identify! tryShooting! loop then ;");
    defaultList.add(": play ( -- ) firstMove shot false ! ( prepare to shoot ) begin edgeMove shootEveryone noMovesLeft? until ; ");
    defaultRobot.put("code", defaultList);
